...
@date     \$Date: 2013/08/02 20:00:05 $

Multi-process version of runpoly. Set $OMP_NUM_THREADS to desired number of 
threads

$Log: runpolymp_KEPQ3.py,v $
Revision 1.1  2013/08/02 20:00:05  paegerm
initial revision
...
from runpoly_estrella_12 import *
...
#     plcreader  = dbr.DbReader(options.dbdir + options.plcname)
    ...
    dictwriter = dbw.DbWriter(options.dbdir + options.dictname, 
                              dbc.dictcols, tout = 60.0, isolevel=None)
...
#         varclsidx = 1 + dbc.dictcols.index(options.clscol)
        ...
        # borders
        dstar = {'staruid': staruid, 'KIC': star[2], 'PERIOD' : star[3]}
        (ok, chi2, coeffs, fit, midpoints) = fitlc(dstar, blc, options)
        try:
...
if __name__ == '__main__':
    ...
    dictreader = dbr.DbReader(options.dbdir + options.dictname, False)
