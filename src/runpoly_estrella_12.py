...
@date     \$Date: 2013/08/02 20:01:42 $

$Log: runpoly_estrella_12.py,v $
Revision 1.1  2013/08/02 20:01:42  paegerm
initial revision
...
def fitlc(star, blc, options):
    os.chdir(options.polydir)
    if options.debug > 0:
        options.lf.write('processing ' + star['KIC'] + ' ' + str(star['staruid']))
...
#     plcmags   = [x[4] for x in plc]
    ...
    resold = p.returncode
    if (resold == 0):
        (oldchi2, oldcoeffs, oldfit) = getfit(outstring, star['staruid'])
    if (options.debug > 1):
    ...
    resnew = p.returncode
    if (resnew == 0):
        (newchi2, newcoeffs, newfit) = getfit(outstring, star['staruid'])
    if (options.debug > 1):
    ...
    if (resold == 0):
        ...
        if (resnew == 0) and (newchi2 < 1.0e6):
            ...
            usefit = None
    if usefit == 'new':
        options.lf.write('using new fit (2 chains) for ' + str(star['staruid']))
    ...
    res = p.returncode
    if (res == 0):
        (tmpchi2, coeffs, fullfit) = getfit(outstring, star['staruid'])
    if (options.debug > 1):
...
#                       help='database file with phased light curves')
    ...
    parser.add_option('--polydir', dest='polydir', type='string', 
                      default='/home/parvizm/workspace/polyfit/',
                      help='directory for polyfit files (default = rootdir)')
    ...
    parser.add_option('--select', dest='select', type='string', 
                      default='select * from stars order by staruid;',
...
#                       help='cmd line options for polyfit entered within ""; example: --polyopt "--step-size 0.005";  (default: None) ')
    ...
    options.lf = Logfile(options.rootdir + options.logfile, True, False)
...
#     plcreader  = dbr.DbReader(options.dbdir + options.plcname)
    ...
    dictwriter = dbw.DbWriter(options.dbdir + options.dictname, 
                              dbc.dictcols, isolevel=None)
...
#         plc = plcreader.getlc(star['uid'], 'stars', 'phase')
        blc = blcreader.fetchall('select * from stars where staruid = ' + str(staruid))
        if blc == None or len(blc) == 0:
            continue
        cffwriter.deletebystaruid(star['staruid'])
        fitwriter.deletebystaruid(star['staruid'])
        midwriter.deletebystaruid(star['staruid'])
...
#                 os.mkdir('failed')
        (ok, chi2, coeffs, fit, midpoints) = fitlc(star, blc, options)
        dictwriter.update('update stars set chi2 = ? where staruid = ?;', 
                          [(chi2, star['staruid'])])
        if ok == False:
            failed += 1
            continue
